import{_ as e}from"./plugin-vue_export-helper-c27b6911.js";import{o as n,c as i,e as s}from"./app-10bfa633.js";const a={},l=s(`<h1 id="_4-4-playbook高级用法" tabindex="-1"><a class="header-anchor" href="#_4-4-playbook高级用法" aria-hidden="true">#</a> 4.4 Playbook高级用法</h1><h2 id="_1-本地执行" tabindex="-1"><a class="header-anchor" href="#_1-本地执行" aria-hidden="true">#</a> 1 本地执行</h2><p>默认情况下，Ansible使用ssh去连接远程主机，但实际上它提供了多种插件来丰富连接方式：smart、ssh、paramiko、local、docker、winrm，默认为smart。</p><p>smart表示智能选择ssh和paramiko(paramiko是Python的一个ssh协议模块)，当Ansible端安装的ssh支持ControlPersist(即持久连接)时自动使用ssh，否则使用paramiko。local和docker是非基于ssh连接的方式，winrm是连接Windows的插件。</p><p>可以在命令行选项中使用-c或--connection选项来指定连接方式：</p><p>如果希望在控制主机本地运行一个特定的任务，可以使用local_action语句。</p><p>假设我们需要配置的远程主机刚刚启动，如果我们直接运行playbook，可能会因为sshd服务尚未开始监听而导致失败，我们可以在控制主机上使用如下示例来等待被控端sshd端口监听：</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>- name: wait for ssh server to be running
  wait_for
      port: 22 
