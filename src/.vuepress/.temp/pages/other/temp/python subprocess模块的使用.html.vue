...
<h2 id="subprocess-call-subprocess-check-call-subprocess-check-output" tabindex="-1"><a class="header-anchor" href="#subprocess-call-subprocess-check-call-subprocess-check-output" aria-hidden="true">#</a> subprocess.call(), subprocess.check_call(), subprocess.check_output()</h2>
<p>执行由参数提供的命令.</p>
...
<p>属性，可用try…except…来检查。</p>
<h3 id="subprocess-check-output" tabindex="-1"><a class="header-anchor" href="#subprocess-check-output" aria-hidden="true">#</a> subprocess.check_output()</h3>
<p>父进程等待子进程完成</p>
<p>返回子进程向标准输出的输出结果</p>
<p>检查退出信息，如果returncode不为0，则举出错误subprocess.CalledProcessError，该对象包含有returncode</p>
...
<h2 id="子进程的文本流控制" tabindex="-1"><a class="header-anchor" href="#子进程的文本流控制" aria-hidden="true">#</a> 子进程的文本流控制</h2>
<p>子进程的标准输入、标准输出和标准错误如下属性分别表示:</p>
<p>代码如下:</p>
<ul>
<li>child.stdin</li>
<li>child.stdout</li>
...
<blockquote>
<p>注意：communicate()是Popen对象的一个方法，该方法会阻塞父进程，直到子进程完成</p>
</blockquote>
<p>wait()方法可能会造成死锁, 那死锁问题如何避免呢？</p>
<p>官方文档里推荐使用 Popen.communicate()。</p>
<p>这个方法会把输出放在内存，而不是管道里，所以这时候上限就和内存大小有关了，一般不会有问题。</p>
<p>而且如果要获得程序返回值，可以在调用 Popen.communicate() 之后取 Popen.returncode 的值。</p>
<blockquote>
<p>结论：如果使用 subprocess.Popen，就不使用 Popen.wait()，而使用 Popen.communicate() 来等待外部程序执行结束</p>
</blockquote>
<blockquote>
