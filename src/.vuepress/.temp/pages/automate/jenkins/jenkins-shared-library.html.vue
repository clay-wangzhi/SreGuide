<template><h1 id="_4-4-jenkins共享库应用" tabindex="-1"><a class="header-anchor" href="#_4-4-jenkins共享库应用" aria-hidden="true">#</a> 4.4 Jenkins共享库应用</h1>
<h2 id="由来与演进" tabindex="-1"><a class="header-anchor" href="#由来与演进" aria-hidden="true">#</a> 由来与演进</h2>
...
并且通过“pipeline script from SCM”方式，可以保存这个流水线的定义，进行版本化管理。</p>
<p>在实际使用中，一开始大都会制作一个尽可能通用的pipeline脚本样例，让搭建者只需要修改几个赋值参数就可以在项目中应用。
但不可避免的是，不同的项目往往有不同的需求，随着定制化需求越来越多，加入了不同的功能实现代码，导致pipeline也会越来越复杂，可读性差，也不利于维护。
而且随着pipeline数目不断的增多，将会发现同一类型和用途的pipeline，它们的很多功能其实是相同的，这时如果对某个功能做改动，往往需要更新数量众多的JenkinsFile。</p>
<p>针对pipeline的扩展和管理问题，共享库功能（Shared Libraries）应运而生。
...
而且通过外部源代码控制（SCM）的方式，保证所有job在构建的时候会调用最新的共享库代码。</p>
<h2 id="共享库是什么" tabindex="-1"><a class="header-anchor" href="#共享库是什么" aria-hidden="true">#</a> 共享库是什么？</h2>
<p>共享库（shared library）是一些<strong>独立的 Groovy 脚本的集合</strong>，我们可以在运行 Pipeline 的时候去获取这些共享库代码。使用共享库最好的方式同样是把代码使用 Git 仓库进行托管，这样我们就可以进行版本化管理了。当然我们也需要一些 Groovy 语言的基础，不过并不需要多深入，基本的语法概念掌握即可，可以查看前面我们的 <a href="https://www.qikqiak.com/post/groovy-simple-tutorial/" target="_blank" rel="noopener noreferrer">Groovy 简明教程<ExternalLinkIcon/></a> 。</p>
...
<h2 id="共享库内容" tabindex="-1"><a class="header-anchor" href="#共享库内容" aria-hidden="true">#</a> 共享库内容</h2>
...
<div class="language-groovy ext-groovy line-numbers-mode"><pre v-pre class="language-groovy"><code><span class="token shebang comment">#!/usr/bin/env groovy</span>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br><span class="line-number">5</span><br><span class="line-number">6</span><br><span class="line-number">7</span><br></div></div><p>我们可以在 Jenkins Pipeline 中使用 <code>import</code> 导入上面的类，并引用其中的静态变量，比如 <code>GlobalVars.foo</code>。</p>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br><span class="line-number">5</span><br></div></div><p>需要注意的是需要实现 call 方法，添加了一个名为 name 的参数，具有默认值 <code>QIKQIAK</code>，可以用 <code>${name}</code> 来进行访问。</p>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br><span class="line-number">5</span><br><span class="line-number">6</span><br><span class="line-number">7</span><br><span class="line-number">8</span><br><span class="line-number">9</span><br><span class="line-number">10</span><br><span class="line-number">11</span><br></div></div><p>完整的代码目录如下所示：</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br><span class="line-number">5</span><br><span class="line-number">6</span><br><span class="line-number">7</span><br><span class="line-number">8</span><br><span class="line-number">9</span><br><span class="line-number">10</span><br><span class="line-number">11</span><br></div></div><p>完整的代码上传到了 GitHub 仓库 https://github.com/cnych/pipeline-library-demo 中。</p>
...
<p><img src="https://clay-blog.oss-cn-shanghai.aliyuncs.com/img/20200520174843.png" alt="Global Pipeline Libraries" loading="lazy">Global Pipeline Libraries</p>
<p>保存后即可使用配置共享库。接下来新建一个名为 <code>share-lib-demo</code> 的流水线项目，在 <code>Pipeline script</code> 区域添加如下代码：</p>
<div class="language-groovy ext-groovy line-numbers-mode"><pre v-pre class="language-groovy"><code><span class="token annotation punctuation">@Library</span><span class="token punctuation">(</span><span class="token string">'pipeline-library-demo'</span><span class="token punctuation">)</span><span class="token number">_</span>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br><span class="line-number">5</span><br><span class="line-number">6</span><br><span class="line-number">7</span><br><span class="line-number">8</span><br><span class="line-number">9</span><br></div></div><p><img src="https://bxdc-static.oss-cn-beijing.aliyuncs.com/images/20200520175022.png" alt="img" loading="lazy"></p>
...
<p><img src="https://bxdc-static.oss-cn-beijing.aliyuncs.com/images/20200520175509.png" alt="img" loading="lazy"></p>
<p>这样我们就完成了一个共享库的编写。</p>
<h2 id="参考链接" tabindex="-1"><a class="header-anchor" href="#参考链接" aria-hidden="true">#</a> 参考链接</h2>
<p><a href="https://www.cnblogs.com/anliven/p/13693871.html" target="_blank" rel="noopener noreferrer">Jenkins - 共享库（Shared Libraries）| anliven | 博客园<ExternalLinkIcon/></a></p>
<p><a href="https://www.qikqiak.com/post/jenkins-shared-library-demo/" target="_blank" rel="noopener noreferrer">Jenkins 共享库示例 | 阳明的博客<ExternalLinkIcon/></a></p>
</template>
