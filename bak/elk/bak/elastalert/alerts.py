...
class BasicMatchString(object):
    ...
    def _add_custom_alert_text(self):
        ...
        if 'alert_text_args' in self.rule:
            ...
            # 下面11行是新增的
            if len(alert_text_values[4]) <= 20:
                UTC_FORMAT = "%Y-%m-%dT%H:%M:%SZ"
            else:
                UTC_FORMAT = "%Y-%m-%dT%H:%M:%S.%fZ"
            utc_time = datetime.datetime.strptime(alert_text_values[4], UTC_FORMAT)
            MAX_LINE = 50
            alert_text_values[4] = utc_time + datetime.timedelta(hours=8)
            logging.warning('4')
            logging.warning(alert_text_values[4])
            listline = alert_text_values[0].splitlines()
            alert_text_values[0] = '\n'.join(listline[:MAX_LINE])
            logging.warning('0')
            #logging.warning(alert_text_values[0])
            #logging.warning(type(alert_text_values[0]))
