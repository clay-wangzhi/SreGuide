...
![640](https://gitee.com/clay-wangzhi/blogImg/raw/master/blogImg/640.png)
...
- SUNCAR私有云：5台部署ES节点，3台分别部署logstash、kafka、zookeeper和kibana等服务。
- SUNCAR私有云配置：5台 4核16G SSD磁盘。3台 4核16G SSD磁盘。都是 Centos7.6系统
- 安装 docker 和 docker-compose
- ELK版本7.10.1；zookeeper版本3.6.2；kafka版本2.13-2.6.0；

| IP地址          | 主机名称   | 用途                                                      |
| :-------------- | :--------- | :-------------------------------------------------------- |
| 192.168.134.6   | es-master1 | es master 和 es 数据节点                                  |
| 192.168.134.103 | es-master2 | es master 和 es 数据节点                                  |
| 192.168.134.168 | es-master3 | es master 和 es 数据节点                                  |
| 192.168.134.240 | es-data1   | es数据节点                                                |
| 192.168.134.185 | es-data2   | es数据节点                                                |
| 192.168.134.162 | logstash1  | logstash、kafka、zookeeper                                |
| 192.168.134.186 | logstash2  | logstash、kafka、zookeeper                                |
| 192.168.134.40  | logstash3  | logstash、kafka、kafa-manager、zookeeper、kibana、curator |
...
> 注意：磁盘分片分配参数，确定数据目录的磁盘空间大于`cluster.routing.allocation.disk.watermark.low`的值
