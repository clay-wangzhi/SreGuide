...
<h2 id="cni-是什么" tabindex="-1"><a class="header-anchor" href="#cni-是什么" aria-hidden="true">#</a> CNI 是什么</h2>
<p>CNI，它的全称是 Container Network Interface，即容器网络的 API 接口。</p>
<p>它是 K8s 中标准的一个调用网络实现的接口。Kubelet 通过这个标准的 API 来调用不同的网络插件以实现不同的网络配置方式。</p>
<h2 id="k8s-中如何使用cni" tabindex="-1"><a class="header-anchor" href="#k8s-中如何使用cni" aria-hidden="true">#</a> k8s 中如何使用CNI</h2>
<p>K8s 通过 CNI 配置文件来决定使用什么 CNI。</p>
<p>基本的使用方法为：</p>
<ol>
<li>首先在每个结点上配置 CNI 配置文件，在 /etc/cni/net.d/ 目录下；</li>
<li>安装 CNI 配置文件中所对应的二进制插件到 /opt/cni/bin/ 目录下；</li>
...
<li>
<p><strong>路由模式</strong>中主机和容器也分属不同的网段，它与 Overlay 模式的主要区别在于它的跨主机通信是通过路由打通，无需在不同主机之间做一个隧道封包。但路由打通就需要部分依赖于底层网络，比如说要求底层网络有二层可达的一个能力；</p>
</li>
<li>
<p><strong>Underlay 模式</strong>中容器和宿主机位于同一层网络，两者拥有相同的地位。容器之间网络的打通主要依靠于底层网络。因此该模式是强依赖于底层能力的</p>
</li>
</ul>
<blockquote>
