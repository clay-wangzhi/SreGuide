<template><div><h1 id="etcd-备份恢复" tabindex="-1"><a class="header-anchor" href="#etcd-备份恢复" aria-hidden="true">#</a> etcd 备份恢复</h1>
<h2 id="etcd-备份" tabindex="-1"><a class="header-anchor" href="#etcd-备份" aria-hidden="true">#</a> etcd 备份</h2>
<ol>
<li>
<p>创建备份脚本<code v-pre>/opt/etcd_backup.sh</code></p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token shebang important">#!/usr/bin/env bash</span>
...
<li>
<p>添加cron定时任务 <code v-pre>crontab -e</code></p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token number">0</span> */1 * * * /bin/bash /opt/etcd_backup.sh <span class="token operator">>></span>/opt/log-backup-etcd.log <span class="token operator"><span class="token file-descriptor important">2</span>></span><span class="token file-descriptor important">&amp;1</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div></li>
</ol>
<h2 id="etcd-恢复" tabindex="-1"><a class="header-anchor" href="#etcd-恢复" aria-hidden="true">#</a> etcd 恢复</h2>
<ol>
<li>
<p>创建备份目录</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token function">mkdir</span> <span class="token parameter variable">-p</span> /opt/k8s_manifests_backup
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div></li>
<li>
<p>停止所有 Master 上 kube-apiserver 服务</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token function">mv</span> /etc/kubernetes/manifests/kube-apiserver.yaml /opt/k8s_manifests_backup/
...
<li>
<p>停止集群中所有 etcd 服务</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token comment"># 记录 etcd --name、--initial-advertise-peer-urls、--data-dir 值</span>
...
<li>
<p>移除所有 etcd 存储目录下数据，不同环境下，存储目录可能不一样，线上存储目录为 <code v-pre>/data/etcd</code></p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token function">mv</span> /data/etcd<span class="token punctuation">{</span>,.bak<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div></li>
<li>
<p>拷贝要恢复的快照到所有 etcd 节点，进行快照恢复
所有 etcd 节点操作，不同节点，传入不同的 <code v-pre>--name、--initial-advertise-peer-urls、--data-dir</code>值</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token assign-left variable">ETCDCTL_API</span><span class="token operator">=</span><span class="token number">3</span> /usr/local/bin/etcdctl snapshot restore /tmp/xxx.db <span class="token punctuation">\</span>
...
<li>
<p>启动集群中所有 etcd</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token function">mv</span> /opt/k8s_manifests_backup/etcd.yaml /etc/kubernetes/manifests/
...
<li>
<p>启动所有 Master 上 kube-apiserver 服务</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token function">mv</span> /opt/k8s_manifests_backup/kube-apiserver.yaml /etc/kubernetes/manifests/
...
<p>参考：</p>
<ul>
<li><a href="https://kubesphere.com.cn/forum/d/3532-etcdkubernetes" target="_blank" rel="noopener noreferrer">Etcd备份数据如何做到完美恢复Kubernetes中误删数据呢<ExternalLinkIcon/></a></li>
</ul>
</div></template>
