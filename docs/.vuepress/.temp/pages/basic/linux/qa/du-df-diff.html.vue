<template><div><h1 id="linux-du、df统计的硬盘使用情况不一致问题" tabindex="-1"><a class="header-anchor" href="#linux-du、df统计的硬盘使用情况不一致问题" aria-hidden="true">#</a> Linux，du、df统计的硬盘使用情况不一致问题</h1>
<p>在运维Linux服务器时，会碰到需要查看硬盘空间的情况，这时候，通常会使用<code v-pre>df -lh</code>命令来检查每个挂载了文件系统的硬盘的总量和已使用量，或者，可以使用<code v-pre>du -sh [directory]</code>命令来统计某个目录下所有文件的空间占用。</p>
<p>在使用<code v-pre>df、du</code>命令时，常常会遇到统计的硬盘使用情况不一致的问题。比如du统计根目录下文件总共大小为2G，而df判断挂载在根目录的硬盘已用空间达到了3G，20G甚至更多。发生这种情况，有以下三种原因：</p>
<h2 id="预留空间" tabindex="-1"><a class="header-anchor" href="#预留空间" aria-hidden="true">#</a> 预留空间</h2>
...
<h2 id="幻影文件-phantom-file" tabindex="-1"><a class="header-anchor" href="#幻影文件-phantom-file" aria-hidden="true">#</a> 幻影文件（phantom file）</h2>
...
<h2 id="未统计到的文件" tabindex="-1"><a class="header-anchor" href="#未统计到的文件" aria-hidden="true">#</a> 未统计到的文件</h2>
