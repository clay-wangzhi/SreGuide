...
<h2 id="python简介" tabindex="-1"><a class="header-anchor" href="#python简介" aria-hidden="true">#</a> Python简介</h2>
<h3 id="python的历史" tabindex="-1"><a class="header-anchor" href="#python的历史" aria-hidden="true">#</a> Python的历史</h3>
<ol>
...
<li>2000年10月16日：Python 2.0发布，增加了完整的<a href="https://zh.wikipedia.org/wiki/%E5%9E%83%E5%9C%BE%E5%9B%9E%E6%94%B6_(%E8%A8%88%E7%AE%97%E6%A9%9F%E7%A7%91%E5%AD%B8)" target="_blank" rel="noopener noreferrer">垃圾回收<ExternalLinkIcon/></a>，提供了对<a href="https://zh.wikipedia.org/wiki/Unicode" target="_blank" rel="noopener noreferrer">Unicode<ExternalLinkIcon/></a>的支持。与此同时，Python的整个开发过程更加透明，社区对开发进度的影响逐渐扩大，生态圈开始慢慢形成。</li>
...
<p>目前我们使用的Python 3.7.x的版本是在2018年发布的，Python的版本号分为三段，形如A.B.C。其中A表示大版本号，一般当整体重写，或出现不向后兼容的改变时，增加A；B表示功能更新，出现新功能时增加B；C表示小的改动（例如：修复了某个Bug），只要有修改就增加C。如果对Python的历史感兴趣，可以阅读名为<a href="http://www.cnblogs.com/vamei/archive/2013/02/06/2892628.html" target="_blank" rel="noopener noreferrer">《Python简史》<ExternalLinkIcon/></a>的网络文章。</p>
<h3 id="python的优缺点" tabindex="-1"><a class="header-anchor" href="#python的优缺点" aria-hidden="true">#</a> Python的优缺点</h3>
<p>Python的优点很多，简单的可以总结为以下几点。</p>
<ol>
...
<h3 id="python的应用领域" tabindex="-1"><a class="header-anchor" href="#python的应用领域" aria-hidden="true">#</a> Python的应用领域</h3>
<p>目前Python在Web应用后端开发、云基础设施建设、DevOps、网络数据采集（爬虫）、自动化测试、数据分析、机器学习等领域都有着广泛的应用。</p>
<h2 id="python-解释器分类" tabindex="-1"><a class="header-anchor" href="#python-解释器分类" aria-hidden="true">#</a> Python 解释器分类</h2>
<table>
<thead>
<tr>
<th>解释器</th>
...
<p>想要开始Python编程之旅，首先得在自己使用的计算机上安装Python解释器环境，下面将以安装官方的Python解释器为例，讲解如何在不同的操作系统上安装Python环境。官方的Python解释器是用C语言实现的，也是使用最为广泛的Python解释器，通常称之为CPython。</p>
<h2 id="安装python解释器" tabindex="-1"><a class="header-anchor" href="#安装python解释器" aria-hidden="true">#</a> 安装Python解释器</h2>
<h3 id="windows环境" tabindex="-1"><a class="header-anchor" href="#windows环境" aria-hidden="true">#</a> Windows环境</h3>
<p>可以在<a href="https://www.python.org" target="_blank" rel="noopener noreferrer">Python官方网站<ExternalLinkIcon/></a>下载到Python的Windows安装程序（exe文件），需要注意的是如果在Windows 7环境下安装Python 3.x，需要先安装Service Pack 1补丁包（可以通过一些工具软件自动安装系统补丁的功能来安装），安装过程建议勾选“Add Python 3.x to PATH”（将Python 3.x添加到PATH环境变量）并选择自定义安装，在设置“Optional Features”界面最好将“pip”、“tcl/tk”、“Python test suite”等项全部勾选上。强烈建议选择自定义的安装路径并保证路径中没有中文。安装完成会看到“Setup was successful”的提示。如果稍后运行Python程序时，出现因为缺失一些动态链接库文件而导致Python解释器无法工作的问题，可以按照下面的方法加以解决。</p>
<p>如果系统显示api-ms-win-crt*.dll文件缺失，可以参照<a href="https://zhuanlan.zhihu.com/p/32087135" target="_blank" rel="noopener noreferrer">《api-ms-win-crt*.dll缺失原因分析和解决方法》<ExternalLinkIcon/></a>一文讲解的方法进行处理或者直接在<a href="https://www.microsoft.com/zh-cn/download/details.aspx?id=48145" target="_blank" rel="noopener noreferrer">微软官网<ExternalLinkIcon/></a>下载Visual C++ Redistributable for Visual Studio 2015文件进行修复；如果是因为更新Windows的DirectX之后导致某些动态链接库文件缺失问题，可以下载一个<a href="https://dl.pconline.com.cn/download/360074-1.html" target="_blank" rel="noopener noreferrer">DirectX修复工具<ExternalLinkIcon/></a>进行修复。</p>
<h3 id="linux环境" tabindex="-1"><a class="header-anchor" href="#linux环境" aria-hidden="true">#</a> Linux环境</h3>
<p>Linux环境自带了Python 2.x版本，但是如果要更新到3.x的版本，可以在<a href="https://www.python.org" target="_blank" rel="noopener noreferrer">Python的官方网站<ExternalLinkIcon/></a>下载Python的源代码并通过源代码构建安装的方式进行安装，具体的步骤如下所示（以CentOS为例）。</p>
<ol>
