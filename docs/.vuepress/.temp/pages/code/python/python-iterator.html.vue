<template><div><h1 id="_3-4-迭代器" tabindex="-1"><a class="header-anchor" href="#_3-4-迭代器" aria-hidden="true">#</a> 3.4 迭代器</h1>
<p>我们已经知道，可以直接作用于<code v-pre>for</code>循环的数据类型有以下几种：</p>
<p>一类是集合数据类型，如<code v-pre>list</code>、<code v-pre>tuple</code>、<code v-pre>dict</code>、<code v-pre>set</code>、<code v-pre>str</code>等；</p>
<p>一类是<code v-pre>generator</code>，包括生成器和带<code v-pre>yield</code>的generator function。</p>
<p>这些可以直接作用于<code v-pre>for</code>循环的对象统称为可迭代对象：<code v-pre>Iterable</code>。</p>
<p>可以使用<code v-pre>isinstance()</code>判断一个对象是否是<code v-pre>Iterable</code>对象：</p>
<div class="language-python line-numbers-mode" data-ext="py"><pre v-pre class="language-python"><code><span class="token operator">>></span><span class="token operator">></span> <span class="token keyword">from</span> collections<span class="token punctuation">.</span>abc <span class="token keyword">import</span> Iterable
...
<div class="language-python line-numbers-mode" data-ext="py"><pre v-pre class="language-python"><code><span class="token operator">>></span><span class="token operator">></span> <span class="token keyword">from</span> collections<span class="token punctuation">.</span>abc <span class="token keyword">import</span> Iterator
...
<span class="token boolean">False</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>生成器都是<code v-pre>Iterator</code>对象，但<code v-pre>list</code>、<code v-pre>dict</code>、<code v-pre>str</code>虽然是<code v-pre>Iterable</code>，却不是<code v-pre>Iterator</code>。</p>
<p>把<code v-pre>list</code>、<code v-pre>dict</code>、<code v-pre>str</code>等<code v-pre>Iterable</code>变成<code v-pre>Iterator</code>可以使用<code v-pre>iter()</code>函数：</p>
...
<span class="token boolean">True</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>你可能会问，为什么<code v-pre>list</code>、<code v-pre>dict</code>、<code v-pre>str</code>等数据类型不是<code v-pre>Iterator</code>？</p>
<p>这是因为Python的<code v-pre>Iterator</code>对象表示的是一个数据流，Iterator对象可以被<code v-pre>next()</code>函数调用并不断返回下一个数据，直到没有数据时抛出<code v-pre>StopIteration</code>错误。可以把这个数据流看做是一个有序序列，但我们却不能提前知道序列的长度，只能不断通过<code v-pre>next()</code>函数实现按需计算下一个数据，所以<code v-pre>Iterator</code>的计算是惰性的，只有在需要返回下一个数据时它才会计算。</p>
<p><code v-pre>Iterator</code>甚至可以表示一个无限大的数据流，例如全体自然数。而使用list是永远不可能存储全体自然数的。</p>
<p><strong>小结</strong></p>
<p>凡是可作用于<code v-pre>for</code>循环的对象都是<code v-pre>Iterable</code>类型；</p>
<p>凡是可作用于<code v-pre>next()</code>函数的对象都是<code v-pre>Iterator</code>类型，它们表示一个惰性计算的序列；</p>
<p>集合数据类型如<code v-pre>list</code>、<code v-pre>dict</code>、<code v-pre>str</code>等是<code v-pre>Iterable</code>但不是<code v-pre>Iterator</code>，不过可以通过<code v-pre>iter()</code>函数获得一个<code v-pre>Iterator</code>对象。</p>
<p>Python的<code v-pre>for</code>循环本质上就是通过不断调用<code v-pre>next()</code>函数实现的，例如：</p>
<div class="language-python line-numbers-mode" data-ext="py"><pre v-pre class="language-python"><code><span class="token keyword">for</span> x <span class="token keyword">in</span> <span class="token punctuation">[</span><span class="token number">1</span><span class="token punctuation">,</span> <span class="token number">2</span><span class="token punctuation">,</span> <span class="token number">3</span><span class="token punctuation">,</span> <span class="token number">4</span><span class="token punctuation">,</span> <span class="token number">5</span><span class="token punctuation">]</span><span class="token punctuation">:</span>
    <span class="token keyword">pass</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>实际上完全等价于：</p>
<div class="language-python line-numbers-mode" data-ext="py"><pre v-pre class="language-python"><code><span class="token comment"># 首先获得Iterator对象:</span>
