<template><div><h1 id="_6-模块" tabindex="-1"><a class="header-anchor" href="#_6-模块" aria-hidden="true">#</a> 6 模块</h1>
...
<p>举个例子，一个<code v-pre>abc.py</code>的文件就是一个名字叫<code v-pre>abc</code>的模块，一个<code v-pre>xyz.py</code>的文件就是一个名字叫<code v-pre>xyz</code>的模块。</p>
<p>现在，假设我们的<code v-pre>abc</code>和<code v-pre>xyz</code>这两个模块名字与其他模块冲突了，于是我们可以通过包来组织模块，避免冲突。方法是选择一个顶层包名，比如<code v-pre>mycompany</code>，按照如下目录存放：</p>
<div class="language-ascii line-numbers-mode" data-ext="ascii"><pre v-pre class="language-ascii"><code>mycompany
├─ __init__.py
├─ abc.py
└─ xyz.py
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>引入了包以后，只要顶层的包名不与别人冲突，那所有模块都不会与别人冲突。现在，<code v-pre>abc.py</code>模块的名字就变成了<code v-pre>mycompany.abc</code>，类似的，<code v-pre>xyz.py</code>的模块名变成了<code v-pre>mycompany.xyz</code>。</p>
<p>请注意，每一个包目录下面都会有一个<code v-pre>__init__.py</code>的文件，这个文件是必须存在的，否则，Python就把这个目录当成普通目录，而不是一个包。<code v-pre>__init__.py</code>可以是空文件，也可以有Python代码，因为<code v-pre>__init__.py</code>本身就是一个模块，而它的模块名就是<code v-pre>mycompany</code>。</p>
...
<div class="language-ascii line-numbers-mode" data-ext="ascii"><pre v-pre class="language-ascii"><code>mycompany
 ...
 ├─ abc.py
 └─ utils.py
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>文件<code v-pre>www.py</code>的模块名就是<code v-pre>mycompany.web.www</code>，两个文件<code v-pre>utils.py</code>的模块名分别是<code v-pre>mycompany.utils</code>和<code v-pre>mycompany.web.utils</code>。</p>
<blockquote>
...
<li>模块名不要和系统模块名冲突，最好先查看系统是否已存在该模块，检查方法是在Python交互环境执行<code v-pre>import abc</code>，若成功则说明系统存在此模块。</li>
</ul>
...
<p>我们以内建的<code v-pre>sys</code>模块为例，编写一个<code v-pre>hello</code>的模块：</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>第1行和第2行是标准注释，第1行注释可以让这个<code v-pre>hello.py</code>文件直接在Unix/Linux/Mac上运行，第2行注释表示.py文件本身使用标准UTF-8编码；</p>
<p>第4行是一个字符串，表示模块的文档注释，任何模块代码的第一个字符串都被视为模块的文档注释；</p>
<p>第6行使用<code v-pre>__author__</code>变量把作者写进去，这样当你公开源代码后别人就可以瞻仰你的大名；</p>
<p>以上就是Python模块的标准文件模板，当然也可以全部删掉不写，但是，按标准办事肯定没错。</p>
<p>后面开始就是真正的代码部分。</p>
<p>你可能注意到了，使用<code v-pre>sys</code>模块的第一步，就是导入该模块：</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token function">import</span> sys
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div><p>导入<code v-pre>sys</code>模块后，我们就有了变量<code v-pre>sys</code>指向该模块，利用<code v-pre>sys</code>这个变量，就可以访问<code v-pre>sys</code>模块的所有功能。</p>
<p><code v-pre>sys</code>模块有一个<code v-pre>argv</code>变量，用list存储了命令行的所有参数。<code v-pre>argv</code>至少有一个元素，因为第一个参数永远是该.py文件的名称，例如：</p>
<p>运行<code v-pre>python3 hello.py</code>获得的<code v-pre>sys.argv</code>就是<code v-pre>['hello.py']</code>；</p>
<p>运行<code v-pre>python3 hello.py Michael</code>获得的<code v-pre>sys.argv</code>就是<code v-pre>['hello.py', 'Michael']</code>。</p>
<p>最后，注意到这两行代码：</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token keyword">if</span> <span class="token assign-left variable">__name__</span><span class="token operator">==</span><span class="token string">'__main__'</span><span class="token builtin class-name">:</span>
    test<span class="token punctuation">(</span><span class="token punctuation">)</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div></div></div><p>当我们在命令行运行<code v-pre>hello</code>模块文件时，Python解释器把一个特殊变量<code v-pre>__name__</code>置为<code v-pre>__main__</code>，而如果在其他地方导入该<code v-pre>hello</code>模块时，<code v-pre>if</code>判断将失败，因此，这种<code v-pre>if</code>测试可以让一个模块通过命令行运行时执行一些额外的代码，最常见的就是运行测试。</p>
<p>我们可以用命令行运行<code v-pre>hello.py</code>看看效果：</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code>$ python3 hello.py
Hello, world<span class="token operator">!</span>
$ python hello.py Michael
Hello, Michael<span class="token operator">!</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>如果启动Python交互环境，再导入<code v-pre>hello</code>模块：</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>导入时，没有打印<code v-pre>Hello, word!</code>，因为没有执行<code v-pre>test()</code>函数。</p>
<p>调用<code v-pre>hello.test()</code>时，才能打印出<code v-pre>Hello, word!</code>：</p>
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token operator">>></span><span class="token operator">></span> hello.test<span class="token punctuation">(</span><span class="token punctuation">)</span>
Hello, world<span class="token operator">!</span>
