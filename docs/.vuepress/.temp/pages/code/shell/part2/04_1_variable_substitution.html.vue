<template><div><h1 id="_4-1-变量替换" tabindex="-1"><a class="header-anchor" href="#_4-1-变量替换" aria-hidden="true">#</a> 4.1 变量替换</h1>
...
<p>接下来我们仔细区分一下<strong>变量名</strong>与<strong>变量值</strong>。如果变量名是 <code v-pre>variable1</code>， 那么 <code v-pre>$variable1</code> 就是对变量值的引用。[^1]</p>
<div class="language-text line-numbers-mode" data-ext="text"><pre v-pre class="language-text"><code>bash$ variable1=23
...
bash$ echo $variable1
23
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>变量仅仅在声明时、赋值时、被删除时（<code v-pre>unset</code>）、被导出时（<code v-pre>export</code>），算术运算中使用双括号结构((...))时或在代表信号时（signal，查看样例 32-5）才不需要有 $ 前缀。赋值可以是使用 =（比如 <code v-pre>var1=27</code>），可以是在 <code v-pre>read</code> 语句中，也可以是在循环的头部（<code v-pre>for var2 in 1 2 3</code>）。</p>
<p>在双引号<code v-pre>&quot;&quot;</code>字符串中可以使用变量替换。我们称之为部分引用，有时候也称弱引用。而使用单引号<code v-pre>''</code>引用时，变量只会作为字符串显示，变量替换不会发生。我们称之为全引用，有时也称强引用。更多细节将在第五章讲解。</p>
<p>实际上, <code v-pre>$variable</code> 这种写法是 <code v-pre>${variable}</code> 的简化形式。在某些特殊情况下，使用 <code v-pre>$variable</code> 写法会造成语法错误，使用完整形式会更好（查看章节 10.2）。</p>
<p>样例 4-1. 变量赋值与替换</p>
...
<div class="language-bash line-numbers-mode" data-ext="sh"><pre v-pre class="language-bash"><code><span class="token builtin class-name">echo</span> <span class="token string">"<span class="token variable">$uninitialized</span>"</span>            <span class="token comment"># 空行</span>
...
<p>[^1]: 实际上，变量名是被称作左值（lvalue），意思是出现在赋值表达式的左侧的值，比如 <code v-pre>VARIABLE=23</code>。变量值被称作右值（rvalue），意思是出现在赋值表达式右侧的值，比如 <code v-pre>VAR2=$VARIABLE</code>。<br />事实上，变量名只是一个引用，一枚指针，指向实际存储数据内存地址的指针。</p>
</div></template>
