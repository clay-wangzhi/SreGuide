...
<h3 id="平铺式目录结构" tabindex="-1"><a class="header-anchor" href="#平铺式目录结构" aria-hidden="true">#</a> 平铺式目录结构</h3>
<p>一个 Go 项目可以是一个应用，也可以是一个代码框架 / 库，当项目是代码框架 / 库时，比较适合采用平铺式目录结构。平铺方式就是在项目的根目录下存放项目的代码，整个目录结构看起来更像是一层的，这种方式在很多框架 / 库中存在，使用这种方式的好处是引用路径长度明显减少</p>
<h3 id="结构化目录结构" tabindex="-1"><a class="header-anchor" href="#结构化目录结构" aria-hidden="true">#</a> 结构化目录结构</h3>
...
<li>
<p>/cmd</p>
<p>一个项目有很多组件，可以把组件 main 函数所在的文件夹统一放在/cmd 目录下</p>
<p>每个组件的目录名应该跟你期望的可执行文件名是一致的。这里要保证 /cmd/&lt;组件名&gt; 目录下不要存放太多的代码，如果你认为代码可以导入并在其他项目中使用，那么它应该位于 /pkg 目录中。如果代码不是可重用的，或者你不希望其他人重用它，请将该代码放到 /internal 目录中。</p>
</li>
<li>
<p>/internal
...
<p>我的建议是，一开始将所有的共享代码存放在 /internal/pkg 目录下，当该共享代码做好了对外开发的准备后，再转存到/pkg目录下。</p>
</li>
<li>
<p>/pkg</p>
...
<li>
<p>/third_party</p>
<p>外部帮助工具，分支代码或其他第三方应用（例如 Swagger UI）。比如我们 fork 了一个第三方 go 包，并做了一些小的改动，我们可以放在目录 /third_party/forked 下。一方面可以很清楚的知道该包是 fork 第三方的，另一方面又能够方便地和 upstream 同步。</p>
</li>
</ol>
<h3 id="go-应用-主要存放测试相关的文件和代码" tabindex="-1"><a class="header-anchor" href="#go-应用-主要存放测试相关的文件和代码" aria-hidden="true">#</a> Go 应用：主要存放测试相关的文件和代码</h3>
<ol start="6">
<li>
<p>/test</p>
<p>用于存放其他外部测试应用和测试数据。/test 目录的构建方式比较灵活：对于大的项目，有一个数据子目录是有意义的。例如，如果需要 Go 忽略该目录中的内容，可以使用 /test/data 或 /test/testdata 目录。</p>
<p>需要注意的是，Go 也会忽略以“.”或 “_” 开头的目录或文件。这样在命名测试数据目录方面，可以具有更大的灵活性。</p>
</li>
</ol>
<h3 id="go-应用-存放跟应用部署相关的文件" tabindex="-1"><a class="header-anchor" href="#go-应用-存放跟应用部署相关的文件" aria-hidden="true">#</a> Go 应用：存放跟应用部署相关的文件</h3>
<ol start="7">
<li>
<p>/configs</p>
...
<h3 id="项目管理-存放用来管理-go-项目的各类文件" tabindex="-1"><a class="header-anchor" href="#项目管理-存放用来管理-go-项目的各类文件" aria-hidden="true">#</a> 项目管理：存放用来管理 Go 项目的各类文件</h3>
<ol start="9">
<li>
<p>/Makefile</p>
<p>虽然 Makefile 是一个很老的项目管理工具，但它仍然是最优秀的项目管理工具。所以，一个 Go 项目在其根目录下应该有一个 Makefile 工具，用来对项目进行管理，Makefile 通常用来执行静态代码检查、单元测试、编译等功能。</p>
</li>
<li>
<p>/scripts</p>
...
<p>另外，shell 脚本中的函数名，建议采用语义化的命名方式，例如 iam::log::info 这种语义化的命名方式，可以使调用者轻松的辨别出函数的功能类别，便于函数的管理和引用。在 Kubernetes 的脚本中，就大量采用了这种命名方式。</p>
</li>
<li>
<p>/build</p>
...
<h3 id="文档-主要存放项目的各类文档" tabindex="-1"><a class="header-anchor" href="#文档-主要存放项目的各类文档" aria-hidden="true">#</a> 文档：主要存放项目的各类文档</h3>
<ol start="14">
<li>
<p>/README.md</p>
<p>项目的 README 文件一般包含了项目的介绍、功能、快速安装和使用指引、详细的文档链接以及开发指引等。</p>
</li>
<li>
<p>/docs</p>
